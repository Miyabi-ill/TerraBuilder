...
namespace TerraBuilder.WorldGeneration.Layers.Biomes
{
    ...
    public class SpawnArea : IWorldGenerationLayer<SpawnArea.SpawnAreaContext>
    {
        ...
        {
            sandbox.WorldSetting.SpawnTileX = sandbox.TileCountX / 2;
            sandbox.WorldSetting.SpawnTileY = sandbox.WorldSetting.RespawnLevel;
            ...
            for (int x = 0; x < sandbox.TileCountX; x++)
            {
                ...
                Coordinate asphaltCoord = new Coordinate(x, sandbox.WorldSetting.RespawnLevel);
                ...
                Coordinate nodestroyCoord = new Coordinate(x, sandbox.WorldSetting.RespawnLevel + 1);
            ...
            Coordinate switchCoord = new Coordinate(5, sandbox.WorldSetting.RespawnLevel - 1);
            Tile switchTile = new Tile() { wall = WallID.Wood };
            switchTile.wire(true);
            _ = sandbox.PlaceTile(switchCoord, switchTile);

            // TODO: sandboxへの呼び出し？
            _ = WorldGen.PlaceTile(5, sandbox.WorldSetting.RespawnLevel - 1, TileID.Switches, forced: true);
